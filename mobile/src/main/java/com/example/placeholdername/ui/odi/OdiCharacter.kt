...
private fun DrawScope.drawCMouth(cx: Float, cy: Float, radius: Float, talkProgress: Float, celebrating: Boolean) {
    val baseSweep = if (celebrating) 200f else 150f
    val sweep = baseSweep + talkProgress * 30f
    val startAngle = 150f + talkProgress * 15f
    val yOffset = if (celebrating) -radius * 0.15f else 0f
    drawArc(
        color = odiBlue,
